...
- Status input comes from committed git history (`HEAD`), not the working tree; `.test-status.json` is trusted, bot-written ledger output
- Developer-owned `renames` and `removals` come from `.tdd-ratchet.json`; successful renames persist in the generated ledger as identity bridges, while removals are transient
